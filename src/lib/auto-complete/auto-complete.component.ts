...
export class AutoCompleteComponent implements OnInit {
  ...
  @ViewChild('autoCompleteDpMultiple') autoCompleteDpMultiple:
    | MultiSelect
    | undefined;
  ...
  ngOnInit(): void {
    this.oldOptions = JSON.parse(
      JSON.stringify(this.columnConfig?.control?.options)
    );
    if (
      this.form.value[
        this.columnConfig?.control?.filterKey
          ? this.columnConfig?.control?.filterKey
          : this.columnConfig?.control?.name
      ]
    ) {
      this.form.value[
        this.columnConfig?.control?.filterKey
        ...
        const option = this.columnConfig?.control?.options.find(
          (option: any) =>
            option[
  ...
  autoCompleteSearch(event: any) {
    const query = this.columnConfig?.control?.multiple
      ? event.filter
      : event.query;
